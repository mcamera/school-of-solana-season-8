...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let result: Option<i64> = OperationType::Addition.perform(x, y);
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let result: Option<i64> = OperationType::Subtraction.perform(x, y);
        if let Some(result) = result {
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let result: Option<i64> = OperationType::Multiplication.perform(x, y);
        if let Some(result) = result {
    ...
    pub fn show_history(&self) -> String {
        ...
        for (idx, operation) in self.history.iter().enumerate() {
